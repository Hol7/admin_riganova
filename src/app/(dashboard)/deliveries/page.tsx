...
export default function DeliveriesPage() {
  ...
  const cancelMutation = useMutation({
    mutationFn: (deliveryId: number) => deliveriesApi.cancelDelivery(deliveryId),
    onSuccess: () => {
  ...
  const handleAssign = () => {
    if (selectedDelivery && selectedLivreurId) {
      assignMutation.mutate({
        deliveryId: selectedDelivery.id,
        livreurId: parseInt(selectedLivreurId),
      });
    } else {
  ...
  const handleCancel = (deliveryId: number) => {
  ...
  const openStatusModal = (delivery: any) => {
    setSelectedDelivery(delivery);
    setNewStatus(delivery.statut);
                    ...
                    return (
                      <tr key={delivery.id} className="hover:bg-gray-50">
                        ...
                        <td className="px-6 py-4">
                          <div className="relative">
                            <button
                              onClick={() => setActiveDropdown(activeDropdown === delivery.id ? null : delivery.id)}
                              ...
                              <div className="absolute right-0 mt-2 w-48 bg-white rounded-xl shadow-lg border border-gray-200 z-50">
                                <div className="py-2">
                                  <button
                                    ...
                                    className="w-full text-left px-4 py-2 text-sm font-montserrat hover:bg-gray-50 flex items-center"
                                  >
                                  ...
                                  {delivery.statut !== 'annule' && delivery.statut !== 'livre' && (
                                    <button
                                      onClick={() => handleCancel(delivery.id)}
                                      className="w-full text-left px-4 py-2 text-sm font-montserrat hover:bg-gray-50 flex items-center text-red-600"
                                    >
                ...
                return (
                  <div key={delivery.id} className="bg-gray-50 rounded-xl p-4">
                    ...
                    <div className="space-y-2 text-sm">
                      ...
                      <div className="font-montserrat font-medium text-gray-700">
                        Livreur: {delivery.livreur?.nom || 'Non assigné'}
                      </div>
                    </div>
                  </div>
                );
          ...
          <div className="bg-white rounded-2xl p-6 w-full max-w-md">
            ...
            <div className="space-y-4">
              ...
              <div className="bg-gray-50 p-4 rounded-xl">
                ...
                <p className="text-sm font-montserrat text-gray-600">
                  <strong>Prix:</strong> {selectedDelivery.prix}FCFA
                </p>
              </div>
            </div>
            ...
            <div className="flex space-x-3 mt-6">
              ...
              <button
                ...
                disabled={assignMutation.isPending || !selectedLivreurId}
                className="flex-1 px-4 py-3 bg-yellow-500 text-white font-montserrat font-semibold rounded-xl hover:bg-yellow-600 disabled:opacity-50"
              >
      ...
      {showStatusModal && selectedDelivery && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl p-6 w-full max-w-md">
            <div className="flex items-center justify-between mb-6">
              <h3 className="text-lg font-montserrat font-bold text-gray-900">
                Modifier le statut - #{selectedDelivery.id}
              </h3>
              <button
                onClick={() => {
                  setShowStatusModal(false);
            ...
            <div className="space-y-4">
              ...
              <div className="bg-gray-50 p-4 rounded-xl">
                <p className="text-sm font-montserrat text-gray-600">
                  ...
                  <span className={`ml-2 px-2 py-1 rounded text-xs font-semibold ${getStatusColor(selectedDelivery.statut)}`}>
                    {selectedDelivery.statut.replace(/_/g, ' ').toUpperCase()}
                  </span>
                </p>
              </div>
            </div>
            ...
            <div className="flex space-x-3 mt-6">
              ...
              <button
                ...
                disabled={statusUpdateMutation.isPending || !newStatus || newStatus === selectedDelivery.statut}
                className="flex-1 px-4 py-3 bg-yellow-500 text-white font-montserrat font-semibold rounded-xl hover:bg-yellow-600 disabled:opacity-50"
              >
                {statusUpdateMutation.isPending ? 'Mise à jour...' : 'Modifier'}
              </button>
            </div>
          </div>
        </div>
